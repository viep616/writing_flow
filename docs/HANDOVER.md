...
- **新交付 02 全流程重跑（2026-09-03，M5 之上）**：上游新交付含 **E_ads 参考计算**（8 基底+5 气体，eads_summary 100 体系全量表）与 **MACE 机器学习线全套**（模型/训练/评估/结论，4.2 meV/atom）。接入：交接区重指新 junction、`qe_extract.attach_eads`（E_ads 逐行复核：重算+与 pwo 交叉，本轮 94 体系零偏差；正值异常 3 个：127/+12.69、115/+1.11、098/+0.22 eV 标记不剔除）、ml_docs 5 件入清单、课题锚定升级 DFT+MACE 双线。全流程 17 分钟跑完：11 阶段全绿、R1/R2 双解析、4 图生成且 M5 图文锚点插入 3 张、PDF 自动出 1196 KB。终态 overall=no **五连后缀**（契约争议+评审未达标+致命一击未过+数值存疑+未通过门），每项有真凭据：契约 contested；R1/R2 均 4-5 分 not_ready（critical：定量断言未在正文嵌入支撑表）；kill FAIL 4 未解；audit 7 findings；**台账 ledger_open=4**（一致性门首因台账拦截）——R2 逐条抓到修订段"说做没做"缺陷（修订说明声称嵌入数据表/收敛判据/ML 数据但正文未落实），修订执行保真列为下一个改进方向（修订说明-实际 diff 一致性的确定性校验）。
- **M5 技能集成与图文结合（2026-09-02，另一会话提交 `409e4c3`）**：qe-analysis-report 技能（4 agent 挂载）、qe_extract §5 分析素材（位次/频次/占比/收敛性代码生成）、md2pdf 图文双通道（FIG 锚点+正文图引用）。
